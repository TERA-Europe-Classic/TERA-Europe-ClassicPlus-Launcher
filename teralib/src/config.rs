...
// PRD 3.1.8 (M6-b): config.json bytes are XOR-obfuscated at build time so
// `strings launcher.exe | grep 192.168.1.128` returns no hits in .rdata.
// The obfuscated blob + per-build 32-byte key live in $OUT_DIR/config_obf.rs
// (generated by build.rs) and are decrypted once on first CONFIG_JSON access.
include!(concat!(env!("OUT_DIR"), "/config_obf.rs"));

fn decrypt_config() -> String {
    let decrypted: Vec<u8> = CONFIG_OBF
        .iter()
        .enumerate()
        .map(|(i, &b)| b ^ CONFIG_KEY[i % CONFIG_KEY.len()])
        .collect();
    String::from_utf8(decrypted).expect("decrypted config must be valid UTF-8")
}

static CONFIG_JSON: Lazy<Value> = Lazy::new(|| {
    let body = decrypt_config();
