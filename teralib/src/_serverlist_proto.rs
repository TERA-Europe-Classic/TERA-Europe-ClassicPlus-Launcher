// This file is @generated by prost-build (proto2).
//
// Hand-vendored to match `src/serverlist.proto` (which uses proto2 syntax
// with `required` modifiers). Regenerating with `prost-build` against the
// proto2 source produces the same attributes — kept checked in so builds
// don't depend on `protoc` being on the host.
