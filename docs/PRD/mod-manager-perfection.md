...
| 3.1.2 | SHA-256 mismatch fail-closes before any disk write for GPK installs | `teralaunch/src-tauri/tests/gpk_install_hash.rs::sha_mismatch_aborts_before_write_gpk` | test passes |
| 3.1.3 | Zip extraction rejects paths escaping the install root | `teralaunch/src-tauri/src/services/mods/external_app.rs::tests::extract_zip_rejects_zip_slip` | test passes with ≥ 3 attack vectors |
| 3.1.4 | TMM deploy path is clamped inside the configured game root | `teralaunch/src-tauri/tests/gpk_deploy_sandbox.rs::deploy_path_clamped_inside_game_root` | test passes with ≥ 5 `..`-based vectors |
| 3.1.5 | HTTP allowlist in `tauri.conf.json` covers every URL the mods code can request; any other URL aborts | `teralaunch/src-tauri/tests/http_allowlist.rs::every_mod_url_on_allowlist` | test passes |
...
| 3.1.7 | All session-sensitive strings (ticket, AuthKey, password) zeroize on drop | `teralaunch/src-tauri/tests/zeroize_audit.rs::all_sensitive_strings_zeroize` | test passes |
...
| 3.2.2 | Launcher crashes mid-install recoverable on next boot | `teralaunch/src-tauri/tests/crash_recovery.rs::mid_install_sigkill_recovers_to_error` | test passes |
| 3.2.3 | `.clean` backup is never overwritten by a subsequent install | `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::clean_backup_not_overwritten_on_second_install` | test passes |
| 3.2.4 | Uninstall everything → mapper byte-for-byte equals `.clean` | `teralaunch/src-tauri/tests/full_cycle.rs::uninstall_all_restores_vanilla_bytes` | test passes (SHA-256 equal) |
...
| 3.2.6 | Catalog parse error caught; filtered entries logged | `teralaunch/tests/catalog-parse.test.js::malformed_entries_filtered` | test passes |
| 3.2.7 | Parallel install of same id serialised (no double-write race) | `teralaunch/src-tauri/tests/parallel_install.rs::same_id_serialised` | test passes |
| 3.2.8 | Disk full mid-install reverses partial writes | `teralaunch/src-tauri/tests/disk_full.rs::revert_on_enospc` | test passes |
| 3.2.9 | `.clean` deleted manually: first deploy recreates if no GPK is currently patched; otherwise refuses with recovery instructions | `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::clean_recovery_logic` | test passes |
| 3.2.10 | Corrupt GPK (magic mismatch) rejected cleanly | `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::parse_mod_file_rejects_non_tmm_gpks` (already passing) | test passes |
...
| 3.3.2 | Per-object GPK merge: two mods patching same composite but different objects both apply | `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::per_object_merge_both_apply` | test passes |
| 3.3.3 | Same (composite, object) conflict surfaces UI warning and last-installed wins | `teralaunch/tests/e2e/mod-conflict-warning.spec.js::conflict_warning_surfaced` | test passes |
| 3.3.4 | "Add mod from file…" accepts a local GPK, parses, verifies, deploys | `teralaunch/tests/e2e/mod-import-file.spec.js::user_imported_gpk_deploys` | test passes |
...
| 3.3.15 | Enable toggle is pure intent (no spawn, no kill); game-launch spawns; game-exit stops | `teralaunch/src-tauri/src/commands/mods.rs::tests::toggle_intent_only` + multi-client tests above | tests pass |
...
| 3.7.4 | No hard-coded English in `teralaunch/src/mods.js`, `mods.html`, `app.js` mod paths | `teralaunch/tests/i18n-no-hardcoded.test.js` (grep-based) | 0 matches |
...
**Security (14 tests):**
- `teralaunch/src-tauri/tests/gpk_install_hash.rs` — criterion 3.1.2; accepts when SHA mismatch returns `Err` with 0 bytes written.
- `teralaunch/src-tauri/tests/gpk_deploy_sandbox.rs` — criterion 3.1.4; accepts with ≥ 5 `..`-based attack vectors blocked.
...
- `teralaunch/src-tauri/src/services/mods/tmm.rs` (cipher / parser / merger)
