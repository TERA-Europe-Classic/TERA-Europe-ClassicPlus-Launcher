...
| 3.1.4 | TMM deploy path is clamped inside the configured game root | `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::deploy_path_clamped_inside_game_root` | test passes with ≥ 5 `..`-based vectors |
...
| 3.2.3 | `.clean` backup is never overwritten by a subsequent install | `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::clean_backup_not_overwritten_on_second_install` (backup stability contract) + `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::golden_cipher_encrypt_zeros_16` (iter-92 §5.4 mapper-cipher byte-for-byte pin — encrypt/decrypt that transforms the backup must stay stable across refactors) | tests pass |
| 3.2.4 | Uninstall everything → mapper byte-for-byte equals `.clean` | `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::uninstall_all_restores_vanilla_bytes` | test passes (SHA-256 equal) |
...
| 3.2.9 | `.clean` deleted manually: first deploy recreates if no GPK is currently patched; otherwise refuses with recovery instructions | `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::clean_recovery_logic_creates_backup_from_vanilla_current` (recreate path) + `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::clean_recovery_logic_refuses_when_current_is_modded` (refuse path); plus 2 companion tests (nop-when-backup-exists, errors-when-mapper-missing) in the same file; plus `teralaunch/src-tauri/tests/clean_recovery.rs::recover_clean_mapper_is_a_tauri_command_and_delegates_to_tmm` (Tauri-command wiring guard) | tests pass |
| 3.2.10 | Corrupt GPK (magic mismatch) rejected cleanly | `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::parse_mod_file_rejects_non_tmm_gpks` (9-fixture adversarial corpus) + `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::golden_v1_fixture_parses_to_expected_modfile` (iter-89 §5.4 parser golden pin — byte-level current-state capture) + `teralaunch/src-tauri/tests/bogus_gpk_footer.rs::parse_mod_file_retains_magic_check_fallback` (iter-79 structural guard — magic-check branch must remain) | tests pass |
...
| 3.3.2 | Per-object GPK merge: two mods patching same composite but different objects both apply | `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::per_object_merge_both_apply` (behavioural) + `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::golden_merger_commutes_on_disjoint_slots` (iter-93 §5.4 merger golden — 2-mod commutativity) + `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::golden_merger_three_disjoint_mods_all_orders_agree` (6-permutation convergence — catches path-dependence that could hide at n=2) | tests pass |
| 3.3.3 | Same (composite, object) conflict surfaces UI warning and last-installed wins | `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::detect_conflicts_flags_other_mod_owning_slot` (Rust predicate — "detect" half) + `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::golden_merger_last_install_wins_on_overlap` (iter-93 — "last-installed wins" half) + `teralaunch/src-tauri/tests/conflict_modal.rs::preview_mod_install_conflicts_is_a_tauri_command_and_delegates_to_tmm` (Tauri-command wiring guard) + `teralaunch/tests/e2e/mod-conflict-warning.spec.js::conflict_warning_surfaced` (IPC contract for UI warning) | tests pass |
...
- `teralaunch/src-tauri/src/services/mods/tmm.rs::tests::deploy_path_clamped_inside_game_root` — criterion 3.1.4; accepts with ≥ 5 `..`-based attack vectors blocked. (Inline: traversal predicate is `is_safe_gpk_container_filename`, guarded at the call site in `install_gpk`.)
